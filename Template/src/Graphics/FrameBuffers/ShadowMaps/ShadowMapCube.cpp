...
ShadowMapCube::ShadowMapCube(GLuint layerIndex, glm::vec3 lightPos, glm::vec3 lightColor, float zNear, float zFar)
    : layerIndex(layerIndex), lightPos(lightPos), farPlane(zFar), color(lightColor)
{
    lightType = LightSystem::LightType::POINT;
...
void ShadowMapCube::BeginDepthPass(unsigned int shaderID, ShadowSystem& shadowSystem)
{
    ...
    shadowSystem.BindDepthTarget(LightSystem::LightType::POINT, layerIndex * 6);
    glEnable(GL_DEPTH_TEST);
    glViewport(0, 0, SHADOW_MAP_WIDTH, SHADOW_MAP_HEIGHT);
    glClear(GL_DEPTH_BUFFER_BIT);
}

void ShadowMapCube::ExportUniformsTo(unsigned int shaderID, int lightIndex)
{
    ShaderManager::Activate(shaderID);
